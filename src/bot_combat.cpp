...
bool CBaseBot::GetEnemy(void)
{
   ...
   if (FNullEnt(pNewEnemy))
   {
      ...
      {
         if (ThreatTab[i].IsUsed == FALSE || ThreatTab[i].IsAlive == FALSE ||
            ThreatTab[i].iTeam == team || ThreatTab[i].pEdict == edict())
            continue;
...
#if 0
         // This is quite senseless in current CS since jumping just reduces
         // shooting accuracy and slow down bot movement
         if (m_iSkill > 80)
         {
            if (m_flJumpTime + 1.5 < gpGlobals->time && (pev->flags & FL_ONGROUND))
            {
               if (RANDOM_LONG(1, 100) < 5 && pev->velocity.Length2D() > 150)
                  pev->button |= IN_JUMP;
            }
         }
#endif
   ...
   if (m_flTimeTeamOrder < gpGlobals->time)
   {
      ...
      for (int ind = 0; ind < gpGlobals->maxClients; ind++)
      {
         if (ThreatTab[ind].IsUsed == FALSE || ThreatTab[ind].IsAlive == FALSE ||
            ThreatTab[ind].iTeam != iTeam || ThreatTab[ind].pEdict == edict())
            continue;
