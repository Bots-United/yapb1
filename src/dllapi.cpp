...
cvar_t g_cvarBotQuota = {"yapb_quota", "0"};
cvar_t g_cvarBotAutoVacate = {"yapb_autovacate", "0"};
cvar_t g_cvarMinBotSkill = {"yapb_minskill", "60"};
cvar_t g_cvarMaxBotSkill = {"yapb_maxskill", "100"};
cvar_t g_cvarMaxNumFollow = {"yapb_followuser", "3"};
cvar_t g_cvarTimeSoundUpdate = {"yapb_timersound", "0.5"};
cvar_t g_cvarTimePickupUpdate = {"yapb_timerpickup", "0.5"};
cvar_t g_cvarTimeGrenadeUpdate = {"yapb_timergrenade", "0.5"};
cvar_t g_cvarDebugGoalIndex = {"yapb_debuggoal", "-1"}; // Assigns a goal in debug mode
cvar_t g_cvarUseExperience = {"yapb_useexp", "1"};
cvar_t g_cvarAutoSaveExperience = {"yapb_autosaveexp", "1"};
cvar_t g_cvarBotChat = {"yapb_botchat", "1"};
cvar_t g_cvarBotUseRadio = {"yapb_botuseradio", "1"};
cvar_t g_cvarJasonMode = {"yapb_jasonmode", "0"};
cvar_t g_cvarDetailNames = {"yapb_detailnames", "1"};
cvar_t g_cvarInstantTurns = {"yapb_inhumanturns", "0"};
cvar_t g_cvarShootThruWalls = {"yapb_shootthruwalls", "1"};
cvar_t g_cvarAllowVotes = {"yapb_votekick", "1"};
cvar_t g_cvarBotSpray = {"yapb_botspray", "1"};
...
void PODBot_ServerCommand(void)
{
   ...
   const char *szUnknownCommand = "YaPB: Unknown command.\n"
      "Type 'yapb ?' for a list of available commands.\n";
   ...
   else if (FStrEq(pcmd, "removerandombot") || FStrEq(pcmd, "kick"))
   {
      ...
      CVAR_SET_FLOAT("yapb_quota", count);
      g_iBotQuota = count;
   }
   ...
   else if (FStrEq(pcmd, "about"))
   {
      SERVER_PRINT(UTIL_VarArgs("=========== VERSION INFORMATION ===========\n"
         "Yet another POD-Bot (BUILD %u, " __DATE__ ")\n"
         "Started from POD-Bot 2.6 by Count Floyd\n"
         "Visit http://yapb.bots-united.com for updates\n"
         "===========================================\n", GetBuildNumber()));
   ...
   {
      SERVER_PRINT("==== Available Server Commands ====\n"
         "yapb about - Display the version information.\n"
         "yapb add - Add a bot.\n"
         "yapb fill - Fill server with bots.\n"
         "yapb removebots - Remove all bots.\n"
         "yapb killbots - Kill all bots.\n"
         "yapb removerandombot - Remove a random bot.\n"
         "yapb newround - Kill all players.\n"
         "yapb wpnmode - Select weapon mode.\n"
         "yapb botsvotemap - Let all dead bots vote for a map.\n");
   }
...
void GameDLLInit( void )
{
   ...
   REG_SVR_COMMAND("yapb", PODBot_ServerCommand);
   ...
   strcat(szFileName, "/addons/yapb/botnames.txt");
   ...
   GetGameDirectory(szFileName);
   strcat(szFileName, "/addons/yapb/botchat.txt");
   ...
   GetGameDirectory(szFileName);
   strcat(szFileName, "/addons/yapb/botskill.cfg");
   ...
   GetGameDirectory(szFileName);
   strcat(szFileName, "/addons/yapb/botlogos.cfg");
   ...
   GetGameDirectory(szFileName);
   strcat(szFileName, "/addons/yapb/botweapons.cfg");
   ...
   SERVER_COMMAND("exec addons/yapb/podbot.cfg\n");
...
void ServerActivate( edict_t *pEdictList, int edictCount, int clientMax )
{
   ...
   g_flBotCheckTime = gpGlobals->time + 2.0;
